  ...
  <ol className="carousel-indicators">
      {
          products.slice(-3).map((product,key)=> (
            <li data-target="#carouselExampleIndicators" data-slide-to={key} key={key} className={`active ${product._id}`}></li>
          ))
      }
  </ol>
  <div className="carousel-inner">
      {
          products.slice(-3).map((product,key) => (
              <Fragment key={key}>
                      <Link className={`carousel-item ${key == 0 ? 'active': ''}`} key={key} style={{width:'100%',height:'400px'}}
